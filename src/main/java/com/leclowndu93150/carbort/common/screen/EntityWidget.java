...
import com.leclowndu93150.carbort.utils.MathUtils;
...
public class EntityWidget extends AbstractWidget {
    ...
    protected void renderWidget(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        if (this.rotationSpeed != 0) {
            this.rotation += partialTick * this.rotationSpeed;
        }

        this.entity.tick();

        GuiUtils.renderEntity(this.entity.getPose().name()., this.entity,
                this.rotationSpeed != 0 ? new Vec3(this.defaultRotation.x(), this.rotation, this.defaultRotation.z())
                        : this.defaultRotation,
                this.scale, this.offset, getX(), getY(), partialTick);
    }
